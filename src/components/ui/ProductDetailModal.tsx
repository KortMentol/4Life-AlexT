 ...
 * @description Модальное окно с двумя режимами: GSAP-анимация для high-tier и Framer Motion для low/medium-tier.
 * @author Kort
 * @version 5.0.0 - Dual mode support
 */
...
const ProductDetailModal = forwardRef<ProductDetailModalHandle, ProductDetailModalProps>(
  ({ product, isOpen, onClose, isGsapControlled = false }, ref) => {
    ...
    useEffect(() => {
      if (isOpen) {
        ...
        setTimeout(() => closeButtonRef.current?.focus(), 600);
      } else {
    ...
    if (isGsapControlled) {
      ...
      return (
        <div className="fixed inset-0 z-[9998] pointer-events-none">
          <div
            ref={panelRef}
            className="fixed inset-0 w-full h-full p-6 md:p-8 opacity-0 pointer-events-none"
        ...
        {isOpen && product && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-6xl max-h-[90vh] bg-gray-900/95 rounded-2xl border border-white/10 overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="relative w-full h-full grid grid-cols-1 lg:grid-cols-2 gap-8 p-8 max-h-[90vh]">
                <div className="absolute top-4 right-4 z-10">
                  <SciFiCloseButton ref={closeButtonRef} onClick={onClose} />
                </div>
                <div
                  className="relative w-full h-64 lg:h-full rounded-2xl"
                  style={{
                    backgroundImage: `url(${product.image})`,
                    backgroundSize: "cover",
                    backgroundPosition: "center",
                ...
                <div className="relative flex flex-col justify-center space-y-6 overflow-y-auto text-white pr-4">
                  <div>
                    <div className="flex flex-wrap gap-2 mb-4">
                      {product.categories.map((category: string) => (
                        <span
