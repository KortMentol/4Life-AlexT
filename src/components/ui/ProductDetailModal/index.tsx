...
const DesktopLayout: React.FC<DesktopLayoutProps> = ({
  ...
  <>
    ...
    <div
      style={{ display: "flex", flex: 1, minHeight: 0, padding: 24, gap: 24 }}
    >
      {/* Media */}
      <div
        style={{
...
>(({ product, products, isOpen, onClose }, ref) => {
  ...
  const gallery = useMemo(
    () => (currentProduct ? buildGallery(currentProduct) : []),
    [currentProduct],
  );
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent("modal-state-change", { detail: { isOpen } }),
    );
  }, [isOpen]);
