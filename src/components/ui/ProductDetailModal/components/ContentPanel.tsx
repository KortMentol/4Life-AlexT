    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-1.5">
        {product.categories.map((cat) => (
          <span
            key={cat}
            className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg ${t.tag}`}
          >
            {cat}
          </span>
        ))}
      ...
      >
        <div>
          <p
            className={`text-[9px] uppercase tracking-widest font-semibold mb-0.5 ${t.subtle}`}
          >
            Цена
          </p>
          <p
      ...
      {product.mainSupport.length > 0 && (
        <div className="space-y-2">
          <p
            className={`text-[10px] font-bold uppercase tracking-widest ${t.subtle}`}
          >
            Поддерживает
          </p>
          <div className="flex flex-wrap gap-1.5">
            {product.mainSupport.map((s) => (
              <span
                key={s}
                className={`px-2.5 py-1 text-xs rounded-lg font-medium ${t.support}`}
              >
                {s}
              </span>
            ))}
      ...
      {product.scientificEvidence && (
        <div className={`p-4 rounded-2xl space-y-1.5 ${t.science}`}>
          <div className="flex items-center gap-2">
            <Icons.FlaskConical
              className={`w-3.5 h-3.5 flex-shrink-0 ${t.scienceLabel}`}
            />
            <span
              className={`text-[10px] font-bold uppercase tracking-widest ${t.scienceLabel}`}
            >
              Научная база
            </span>
          </div>
          <p className={`text-xs leading-relaxed ${t.scienceText}`}>
            {product.scientificEvidence}
          </p>
        </div>
      )}
      ...
      {product.composition && (
        <div className="space-y-1.5 pb-2">
          ...
          <p className={`text-[11px] leading-relaxed ${t.composition}`}>
            {product.composition}
          </p>
        </div>
      )}
      ...
      {!isMobile && (
        <div className={`pt-4 border-t ${t.border}`}>
          ...
          <p className={`text-center text-xs mt-2 ${t.subtle}`}>
