...
const LINES = [
  { text: "Один разговор.", delay: 0, isAccent: false },
  { text: "Никаких обязательств.", delay: 0.1, isAccent: false },
  { text: "Только ясность.", delay: 0.2, isAccent: true },
] as const;
...
const Invitation = memo(
  forwardRef<HTMLElement, InvitationProps>(({ tier, palette, onChapter }, forwardedRef) => {
    ...
    const contentInView = useInView(activeRef, { once: true, margin: "-15%" });
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto w-full">
          ...
          >
            Расскажи о своих целях — через 30 минут у тебя будет точная картина: как модель работает, что ты получаешь,
            и подходит ли это именно тебе.
          </motion.p>
          ...
          >
            Бесплатная консультация · Без давления · Только факты
          </motion.p>
        </div>
      </section>
    );
